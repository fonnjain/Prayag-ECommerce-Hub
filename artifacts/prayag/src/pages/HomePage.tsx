    ...
    <div className="flex items-end justify-between mb-6">
      <div>
        <h2 className="text-2xl font-black text-gray-900 flex items-center gap-2">
          {Icon && <Icon className="w-5 h-5 text-[hsl(215,100%,34%)]" />}
          {title}{" "}
          {accent && <span className="text-gradient-blue">{accent}</span>}
        </h2>
        <div className="w-12 h-1 bg-gradient-to-r from-[hsl(215,100%,34%)] to-transparent mt-2 rounded-full" />
      </div>
      {href && (
  ...
  return (
    <div className="bg-gray-50 min-h-screen overflow-x-hidden">
      ...
      <section className="relative bg-gradient-to-br from-[hsl(222,60%,12%)] via-[hsl(215,100%,20%)] to-[hsl(210,100%,28%)] overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-16 md:py-20 grid lg:grid-cols-2 gap-10 items-center">
          ...
          <div className="text-white">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
              className="inline-flex items-center gap-2 glass rounded-full px-4 py-1.5 text-xs font-semibold mb-5">
              <Droplets className="w-3.5 h-3.5 text-blue-300" /> India's Premier Plumbing Brand
            </motion.div>
            <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}
              className="text-4xl md:text-6xl font-black leading-[1.05] mb-5">
              Complete Bathroom &<br />Plumbing{" "}
              <span className="relative inline-block">
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-300 via-cyan-200 to-blue-400 animate-gradient">Solutions</span>
                <motion.span initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.8, delay: 0.7 }}
                  className="absolute left-0 -bottom-1 w-full h-1 bg-gradient-to-r from-blue-400 to-transparent origin-left rounded-full" />
              </span>
            </motion.h1>
            ...
            <div className="grid grid-cols-3 gap-4 max-w-md">
              {[
                ...
                <motion.div key={stat.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 + i * 0.1 }}
                  ...
                  <div className="text-xl md:text-2xl font-black text-white"><Counter to={stat.n} suffix={stat.s} /></div>
                  <div className="text-[10px] text-blue-200 uppercase tracking-wide">{stat.label}</div>
                </motion.div>
              ))}
            </div>
          </div>
          ...
          <div className="hidden lg:flex justify-center"
            ...
            <motion.div style={{ rotateX: rx, rotateY: ry, transformPerspective: 900 }}
              ...
              <div className="absolute -top-3 -right-3 bg-gradient-to-r from-amber-400 to-orange-500 text-white text-[11px] font-bold px-3 py-1 rounded-full shadow-lg flex items-center gap-1">
              ...
              <Link href="/products?category=cp-faucets">
                <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                  className="shimmer-hover w-full bg-gradient-to-r from-[hsl(215,100%,34%)] to-[hsl(210,100%,42%)] text-white text-sm font-bold py-3 rounded-xl flex items-center justify-center gap-1.5">
        ...
        {/* wave divider */}
        <svg className="relative block w-full h-12 md:h-16" viewBox="0 0 1440 80" preserveAspectRatio="none">
          <path fill="rgb(249 250 251)" d="M0,40 C240,80 480,0 720,30 C960,60 1200,10 1440,40 L1440,80 L0,80 Z" />
        </svg>
      </section>
        ...
        <div className="flex whitespace-nowrap animate-marquee">
          {[...marqueeWords, ...marqueeWords].map((w, i) => (
            <span key={i} className="inline-flex items-center text-white/90 text-sm font-bold mx-6 uppercase tracking-wider">
              <Droplets className="w-3.5 h-3.5 mr-2 text-blue-300" /> {w}
            </span>
          ))}
        </div>
      </div>
                ...
                {(featured || []).slice(0, 4).map(p => (
                  <Link key={p.id} href={`/products/${p.slug}`}>
                    <motion.div whileHover={{ y: -6 }} className="bg-white border border-gray-100 rounded-2xl overflow-hidden hover:shadow-xl hover:border-[hsl(215,100%,34%)]/40 transition-shadow" data-testid={`card-bestseller-${p.id}`}>
                      <div className="relative overflow-hidden">
                        <span className="absolute top-2 left-2 bg-amber-500 text-white text-[9px] font-bold px-2 py-0.5 rounded-full z-10">Bestseller</span>
                      ...
                      <div className="p-3">
                        ...
                        <div className="flex items-center gap-1 mb-1.5">{[...Array(4)].map((_, i) => <Star key={i} className="w-2.5 h-2.5 fill-amber-400 text-amber-400" />)}<span className="text-[9px] text-gray-400">({Math.floor(Math.random() * 200 + 50)})</span></div>
                    ...
                    <Link href={`/products/${p.slug}`}>
                      <div className="flex items-center gap-3 bg-white border border-gray-100 rounded-2xl p-2.5 hover:border-[hsl(215,100%,34%)]/40 hover:shadow-md transition-all group" data-testid={`card-new-arrival-${p.id}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-1 mt-0.5">{[...Array(4)].map((_, i) => <Star key={i} className="w-2.5 h-2.5 fill-amber-400 text-amber-400" />)}</div>
                        </div>
      ...
      <section className="relative bg-gradient-to-br from-[hsl(222,60%,10%)] via-[hsl(215,100%,18%)] to-[hsl(210,100%,24%)] py-14 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 grid md:grid-cols-2 gap-10 items-center">
          <Reveal>
            <div className="text-xs font-bold text-blue-300 uppercase tracking-widest mb-3">Become A Partner</div>
            <h2 className="text-3xl md:text-4xl font-black text-white mb-3">Become A <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-300 to-cyan-200">Prayag Dealer</span></h2>
      ...
      {/* ══════════ NEWSLETTER ══════════ */}
      <section className="bg-[hsl(215,100%,34%)] py-8">
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-white flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl glass flex items-center justify-center flex-shrink-0"><Droplets className="w-6 h-6 text-white" /></div>
            <div>
              <div className="font-black text-base">Subscribe to our newsletter</div>
              <div className="text-blue-200 text-xs">Get the latest updates on new products & offers</div>
            </div>
          </div>
          <div className="flex gap-2 w-full sm:w-auto">
            <input type="email" placeholder="Enter your email address" className="flex-1 sm:w-72 border-0 rounded-xl px-4 py-3 text-sm outline-none text-gray-900" />
            <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }} className="shimmer-hover bg-gray-900 text-white font-bold px-6 py-3 rounded-xl text-sm whitespace-nowrap">Subscribe</motion.button>
          </div>
        </div>
      </section>

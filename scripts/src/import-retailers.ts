...
async function main() {
  ...
  const col = {
    ...
    leadStatus: idx("Lead Status"),
    accountStatus: idx("Status"),
    id: idx("Id"),
    ...
    aadharNo: idx("Aadhar No."),
    aadharFront: idx("Aadhar Front Image"),
    aadharBack: idx("Aadhar Back Image"),
    panNo: idx("Pan No."),
    panImage: idx("Pan Image"),
    bankName: idx("Bank Name"),
    accountHolder: idx("Account Holder Name"),
    accountNo: idx("Account No."),
    ifsc: idx("IFSC Code"),
    bankPassbook: idx("Bank Passbook Image"),
    assignDistributor: idx("Assign Distributor Name"),
  };
  ...
  const values = rows.slice(1).flatMap((r) => {
    ...
    return [{
      ...
      aadharNo: clean(r[col.aadharNo]),
      aadharFrontUrl: clean(r[col.aadharFront]),
      aadharBackUrl: clean(r[col.aadharBack]),
      panNo: clean(r[col.panNo]),
      panImageUrl: clean(r[col.panImage]),
      bankName: clean(r[col.bankName]),
      accountHolderName: clean(r[col.accountHolder]),
      accountNo: clean(r[col.accountNo]),
      ifscCode: clean(r[col.ifsc]),
      bankPassbookUrl: clean(r[col.bankPassbook]),
      assignDistributor: clean(r[col.assignDistributor]),
    }];
  });

  console.log(`Parsed ${values.length} retailer rows`);
  const BATCH = 500;
  for (let i = 0; i < values.length; i += BATCH) {
    await db.insert(distributorsTable).values(values.slice(i, i + BATCH));
    if ((i / BATCH) % 20 === 0 || i + BATCH >= values.length) console.log(`Inserted ${Math.min(i + BATCH, values.length)}/${values.length}`);
  }
  console.log("Done");
  process.exit(0);
}
